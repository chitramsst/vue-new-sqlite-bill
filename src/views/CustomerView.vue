<template>
        ...
        <div class="h-[7%]  shadow-sm  border-b-[0.5px] border-border flex flex-row justify-between p-5 items-center">
            <div class=" text-sm">
                <input type="text" class="border-[0.5px] border-border rounded-lg ring-0 py-2 px-3 w-[100%] outline-0"
                    placeholder="search customer" v-model="search" />
            </div>
                                ...
                                <tbody>
                                    <tr v-for="item in itemResults" :key="item.id">
                                        ...
                                        <td class="px-5 py-5 border-b border-slate-200 bg-white text-sm text-right ">
                                            ...
                                            <CustomerModal :isVisible="isEditModalVisible"
                                                @close="isEditModalVisible = false" title="Edit Customer" :isEdit="true" />
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
    ...
    <CustomerModal :isVisible="isModalVisible" @close="isModalVisible = false" title="Create Customer" :isEdit="false"
        @save="handleUpdates" />
    <CustomerModal :isVisible="isEditModalVisible" @close="isEditModalVisible = false" ref="editmodal" title="Edit Customer"
...
export default {
    ...
    methods: {
        async getItems() {
            window.ipcRenderer.invoke('database-function', { target: 'get-customers' }).then((response) => {
        ...
        deleteItem(item) {
            this.$emitter.emit('show-delete-modal', {
                message: 'Do you want to delete selected customer?',
                ...
                callback: (e: 'CONFIRM' | 'CLOSE') => {
                    if (e == 'CONFIRM') {
                        window.ipcRenderer.invoke('database-function', {
                            ...
                            //         title : 'Delete Failed',
                            //         subtitle : this.$t('Expense Customer cannot be deleted as its used in a expense.'),
                            //         type : 'error'
                            //     })
                            //     return;
                            // }
                            ...
                            //     subtitle: 'Customer has been deleted',
                            //     type: 'success'
                            // })
                        });
                    }
